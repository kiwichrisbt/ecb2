...
{if $is_sub_field}
    {if is_null($sub_row_number)}{* output template field *}
        ...
        <input type="checkbox" name="" id="" value="1" class="cms_checkbox {if $inline_label}inline-label{/if} repeater-field" data-repeater="#{$block_name}-repeater" data-field-name="{$block_name}">
        {if $inline_label}<label for="">{$inline_label}&nbsp;</label>{/if}

    {else}    
        <input type="hidden" name="{$subFieldName}" value="0">
        <input type="checkbox" name="{$subFieldName}" id="{$subFieldId}" value="1" class="cms_checkbox repeater-field {if $inline_label}inline-label{/if}" {if $value}checked="checked"{/if}>
