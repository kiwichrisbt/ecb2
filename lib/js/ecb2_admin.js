...
$(function() {
   ...
   // ecb_repeater
    $('.ecb_repeater.sortable').sortable({placeholder: 'repeater-placeholder'});
    $(document).on('click', '.ecb2-repeater-add', function(e) {
        e.preventDefault();
        var $repeater = $(this).closest('.ecb_repeater'),
            $newRepeaterWrapper = $repeater.find('.repeater-wrapper').first().clone();
        $newRepeaterWrapper.find('.repeater-field').val('').attr('name', $repeater.data('block-name')+'[]');
    ...
    $(document).on('click', '.ecb2-repeater-remove', function(e) {
        ...
        if ( $repeater.children('.repeater-wrapper').length > 1 ) {
            $repeaterWrapper.remove();
        } else {
            $repeaterWrapper.children('.repeater-field').val('');   // just set to '' if last field
        }
        updateECB2RepeaterMaxBlocks($repeater);
    });
