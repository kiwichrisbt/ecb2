...
$(function() {
   ...
   $('.ecb_compact .ecb_select_edit').click( function(e) {
    ...
    $(document).on('click', '.ecb2-repeater-add', function(e) {
        ...
        var $newRepeaterWrapper = $repeater.find('.repeater-wrapper-template').clone();
        $newRepeaterWrapper.removeClass('repeater-wrapper-template')
            .addClass('repeater-wrapper')
            .css('display', '')
            .find('.repeater-field')
                .attr('name', $repeater.data('block-name')+'[]')
                .uniqueId();     // a jquery UI function - add unique id
        if ( $repeater.hasClass('wysiwyg') ) {
    ...
    $(document).on('click', '.ecb2-repeater-remove', function(e) {
        ...
        if ( $repeater.hasClass('wysiwyg') ) { 
            ECB2RepeaterEditorRemove ($repeater, $repeaterWrapper);
        } else if ($repeater.children('.repeater-wrapper').length > 1) {
            $repeaterWrapper.remove();
        } else {
            $repeaterWrapper.children('.repeater-field').val('');   // just clear contents if last field
        }
        updateECB2RepeaterMaxBlocks($repeater);
    });
    // Editor functions support MicroTiny or TinyMCE wysiwyg
    function ECB2RepeaterEditorAdd ($repeater, $newRepeaterWrapper) {
        var $newTinyMceField = $newRepeaterWrapper.find('textarea');
        $newTinyMceField.attr('id','').uniqueId();  // a jquery UI function - add unique id
        var newId = $newTinyMceField.attr('id');
        $repeater.append($newRepeaterWrapper);
        tinymce.EditorManager.execCommand('mceAddEditor', true, newId);
        tinymce.activeEditor.show();

    }
    function ECB2RepeaterEditorRemove ($repeater, $repeaterWrapper) {
        var editorId = $repeaterWrapper.find('textarea').attr('id');
        if ( $repeater.children('.repeater-wrapper').length > 1 ) { // delete repeater editor
